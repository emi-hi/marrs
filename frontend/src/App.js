...
function App() {
  ...
  const axiosSearch = (searchValue) => {
    axios.get(`/api/product?search=${searchValue}`).then((response) => {
  ...
  const refreshProducts = () => {
    setLoading(true);
    axios
      .all([axios.get(`/api/product`), axios.get(`/api/featured-product`)])
      .then(
        axios.spread(
          (productsRes, featuredRes) => (
